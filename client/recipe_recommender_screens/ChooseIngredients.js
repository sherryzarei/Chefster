...
export default function ChooseIngredients({ navigation }) {
    ...
    const handleFindRecipe = async () => {
        ...
        const queryURL = `http://${IP_ADDRESS}:6000/recipes_filter?cookingTime=${cookingTime}&mealType=${mealType}&dietType=${dietType}&ingredients=${encodeURIComponent(ingredientsQuery)}`;
    ...
    const handleSearch = async (query) => {
        ...
        try {
            const response = await fetch(
                `http://${IP_ADDRESS}:6000/ingredients?ingredient=${encodeURIComponent(query)}`
            );
            ...
            <View style={styles.container}>
                ...
                <View style={styles.bodyContainer}>
                    <View style={styles.titleRow}>
                        ...
                        <Text style={styles.headerTitle}>Please choose your ingredients:</Text>
                    </View>
                    <View style={styles.searchContainer}>
                        ...
                        />
                        <TouchableOpacity onPress={() => alert('Search clicked')} style={styles.searchIcon}>
                            <FontAwesome5 name="search" size={20} color="black" />
                        </TouchableOpacity>
                        {searchQuery && filteredIngredients.length > 0 && (
                            <View style={{ height: 240, backgroundColor: '#d9d7d7', borderColor: 'black', borderWidth: 2, borderRadius: 10, padding: 10, marginTop: 10 }}>
                                {filteredIngredients.map((item, index) => (
                                    ...
                                    >
                                        <View style={styles.suggestionRow}>
                                            {renderIcon('flask')}
                                            <Text style={styles.suggestionText}>{formatIngredientName(item)}</Text>
                                        </View>
                                    </TouchableOpacity>
                                ))}
                            </View>
                    ...
                    {selectedIngredients.map((item) => (
                        <View style={styles.row} key={item}>
                            <View style={styles.row}>
                                ...
                                <Text style={styles.headerTitle}>{formatIngredientName(item)}</Text>
                            </View>
...
const styles = StyleSheet.create({
    ...
    button: { backgroundColor: 'black', padding: 12, borderRadius: 8, marginTop: 16, marginBottom: 20 },
    ...
    searchContainer: { position: 'relative', width: '100%', justifyContent: 'center', marginBottom: 10 },
    searchBar: { height: 40, borderColor: 'black', borderWidth: 3, borderRadius: 10, paddingLeft: 10, paddingRight: 10, fontSize: 16, shadowColor: 'black', shadowOffset: { width: 8, height: 8 }, shadowOpacity: 0.8, shadowRadius: 8, elevation: 40 },
    searchIcon: { position: 'static', right: 10, top: '50%', transform: [{ translateY: -30 }, { translateX: 280 }] },
    suggestionItem: { padding: 10, borderBottomWidth: 2, borderBottomColor: 'black' },
    suggestionRow: { flexDirection: 'row', alignItems: 'center' },
    suggestionText: { marginLeft: 10, fontSize: 16 },
    ingredientList: { marginTop: 20, backgroundColor: '#edf0f5', borderColor: 'black', borderWidth: 3, borderRadius: 10, marginVertical: 10, shadowColor: 'black', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 4, elevation: 5, padding: 15 },
