...
const EditPreferences = ({ uid }) => {
    ...
    useEffect(() => {
        const fetchPreferences = async () => {
            try {
                const response = await axios.get(`http://${IP_ADDRESS}:6000/users/certain_user/${userID}`);
    ...
    const handleSavePreferences = async () => {
        try {
            await axios.put(`http://${IP_ADDRESS}:6000/users/update_user/${userID}`, { foodAllergies, dietType });
